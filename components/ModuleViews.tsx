...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: Record<string, string> = {
    ...
    'Allocated': 'bg-purple-50 text-purple-700 ring-purple-600/20',
    ...
    'Upcoming': 'bg-indigo-50 text-indigo-700 ring-indigo-600/20',
  ...
  const colorClass = styles[status] || 'bg-slate-50 text-slate-600 ring-slate-500/10';

  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ring-1 ring-inset ${colorClass}`}>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm text-left">
        <thead className="bg-slate-50/80 border-b border-slate-100">
          <tr>
            {headers.map((h, i) => (
              <th key={i} className={`px-6 py-4 font-semibold text-slate-500 text-xs uppercase tracking-wider ${i === headers.length - 1 ? 'text-right' : ''}`}>
                {h}
              </th>
            ))}
          </tr>
