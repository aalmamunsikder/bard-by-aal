...
import { BardAssistant } from './components/BardAssistant';
import { ModuleContent } from './components/ModuleViews';
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <aside 
        ...
        } bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-slate-300 flex-shrink-0 transition-all duration-500 ease-in-out flex flex-col fixed md:relative h-full z-50 shadow-2xl border-r border-slate-700/50`}
      >
        {/* Sidebar Header */}
        <div className="h-20 flex items-center justify-center border-b border-slate-700/50 backdrop-blur-sm">
          {sidebarOpen ? (
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50/50">
        ...
        <header className="h-20 absolute top-0 left-0 right-0 z-40 px-8 flex items-center justify-between glass border-b border-slate-200/50">
        ...
        <main className="flex-1 overflow-y-auto pt-24 px-8 pb-10 scroll-smooth">
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                  {[
                    ...
                    return (
                        <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-soft hover:shadow-lg transition-all duration-300 group">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`p-3 rounded-2xl ${colors[stat.color]} transition-colors`}>
                                <Icon size={24} />
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                      {MOCK_ACTIVITY_DATA.map((activity, i) => (
                        <div key={activity.id} className="relative pl-6 pb-2 group">
                          ...
                          <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-white shadow-sm z-10 ${
